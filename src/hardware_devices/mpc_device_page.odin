...
createDevicePage :: proc(display: ^graphics.Display, daw: ^daw.DAW) -> ^DevicePage {
    ...
    device_page.onAfterLoad = device_page_load
...
device_page_load :: proc(page: rawptr, route_data: any) {
    device_page := cast(^DevicePage)page
    fmt.printfln("Device page loaded")
    daw := device_page.daw
    track_name_label := device_page.labels["track_name"]
    device_page->addConnection(track_name_label.onDrag, proc(value_type: any, ptr: rawptr) {
        value := value_type.(f64)
        track_name_label := cast(^graphics.LabelElement)ptr
        device_page := cast(^DevicePage)ptr
        device_page.daw.tracks->selectedTrackByEncoder(-value)
    })
}
...
device_page_layout :: proc(page: rawptr) -> clay.ClayArray(clay.RenderCommand) {
    using clay
    device_page := cast(^DevicePage)page
    song_position_label := device_page.labels["song_position"]
    song_pos := device_page.daw.transport->getSongPosition()
    song_position_label->setText(song_pos->toShortString())
    BeginLayout()
    if UI()({
        layout = {
